...
var SelectQueryBuilder = /** @class */ (function (_super) {
    ...
    SelectQueryBuilder.prototype.setFindOptions = function (findOptions) {
        ...
        this.findOptions = {
            ...
            pagination: normalizedFindOptions.options &&
                normalizedFindOptions.options.pagination,
            loadRelationIds: normalizedFindOptions.options &&
    ...
    SelectQueryBuilder.prototype.addSelect = function (selection, selectionAliasName) {
        if (!selection)
            return this;
        if (Array.isArray(selection)) {
            this.expressionMap.selects = this.expressionMap.selects.concat(selection.map(function (selection) { return ({
                selection: selection
            }); }));
        }
        else if (selection instanceof Function) {
    ...
    SelectQueryBuilder.prototype.loadRelationIdAndMap = function (mapToProperty, relationName, aliasNameOrOptions, queryBuilderFactory) {
        ...
        if (aliasNameOrOptions instanceof Object &&
    ...
    SelectQueryBuilder.prototype.loadAllRelationIds = function (options) {
        ...
        this.expressionMap.mainAlias.metadata.relations.forEach(function (relation) {
            if (options !== undefined &&
                ...
                return;
            _this.loadRelationIdAndMap(_this.expressionMap.mainAlias.name +
                "." +
                relation.propertyPath, _this.expressionMap.mainAlias.name +
                "." +
    ...
    SelectQueryBuilder.prototype.where = function (where, parameters) {
        ...
        if (where &&
            typeof where === "object" &&
            !(where instanceof Brackets_1.Brackets)) {
    ...
    SelectQueryBuilder.prototype.orderBy = function (sort, order, nulls) {
        ...
        if (sort) {
            if (sort instanceof Object) {
                ...
                if (nulls) {
                    this.expressionMap.orderBys = (_a = {},
                        _a[sort] = { order: order, nulls: nulls },
                        _a);
                }
                else {
    ...
    SelectQueryBuilder.prototype.join = function (direction, entityOrProperty, aliasName, condition, parameters, mapToProperty, isMappingMany) {
        ...
        if (joinAttribute.metadata) {
            ...
            var isSubQuery = entityOrProperty instanceof Function ||
    ...
    SelectQueryBuilder.prototype.createSelectExpression = function () {
        ...
        var froms = this.expressionMap.aliases
            ...
            return (_this.getTableName(alias.tablePath) +
                " " +
                _this.escape(alias.name));
        });
        ...
        var selection = allSelects
            ...
            return select.selection +
                (select.aliasName
                    ? " AS " + _this.escape(select.aliasName)
                    : "");
        })
            .join(", ");
    ...
    SelectQueryBuilder.prototype.createSelectDistinctExpression = function () {
        ...
        if (driver instanceof PostgresDriver_1.PostgresDriver && selectDistinctOn.length > 0) {
            var selectDistinctOnMap = selectDistinctOn
                .map(function (on) { return _this.replacePropertyNames(on); })
                .join(", ");
    ...
    SelectQueryBuilder.prototype.createJoinExpression = function () {
        ...
        var joins = this.expressionMap.joinAttributes.map(function (joinAttr) {
            ...
            if (!parentAlias || !relation) {
                var destinationJoin = joinAttr.alias.subQuery
                    ? joinAttr.alias.subQuery
                    : _this.getTableName(destinationTableName);
                return (" " +
                    joinAttr.direction +
                    " JOIN " +
                    destinationJoin +
                    " " +
                    _this.escape(destinationTableAlias) +
                    (joinAttr.condition
                        ? " ON " + _this.replacePropertyNames(joinAttr.condition)
                        : ""));
            }
            // if real entity relation is involved
            if (relation.isManyToOne || relation.isOneToOneOwner) {
                // JOIN `category` `category` ON `category`.`id` = `post`.`categoryId`
                var condition = relation.joinColumns
                    .map(function (joinColumn) {
                ...
                return (" " +
                    ...
                    _this.escape(destinationTableAlias) +
                    " ON " +
                    _this.replacePropertyNames(condition + appendedCondition));
            }
            else if (relation.isOneToMany || relation.isOneToOneNotOwner) {
                // JOIN `post` `post` ON `post`.`categoryId` = `category`.`id`
                var condition = relation
                    .inverseRelation.joinColumns.map(function (joinColumn) {
                    return (destinationTableAlias +
                        "." +
                ...
                // means many-to-many
                var junctionTableName = relation.junctionEntityMetadata
                    .tablePath;
                ...
                if (relation.isOwning) {
                    ...
                    junctionCondition = relation
                        .inverseRelation.inverseJoinColumns.map(function (joinColumn) {
                ...
                return (" " +
                    joinAttr.direction +
                    " JOIN " +
                    _this.getTableName(junctionTableName) +
                    " " +
                    _this.escape(junctionAlias_1) +
                    " ON " +
                    _this.replacePropertyNames(junctionCondition) +
                    " " +
                    joinAttr.direction +
                    " JOIN " +
                    _this.getTableName(destinationTableName) +
                    ...
                    _this.escape(destinationTableAlias) +
                    " ON " +
                    _this.replacePropertyNames(destinationCondition + appendedCondition));
            }
        });
        return joins.join(" ");
    };
            ...
            return (" ORDER BY " +
                Object.keys(orderBys)
                    ...
                    if (typeof orderBys[columnName] === "string") {
                        return (_this.replacePropertyNames(columnName) +
                            " " +
                            orderBys[columnName]);
                    }
                    else {
        ...
        switch (this.expressionMap.lockMode) {
            case "pessimistic_read":
                if (driver instanceof MysqlDriver_1.MysqlDriver ||
                    driver instanceof AuroraDataApiDriver_1.AuroraDataApiDriver) {
            ...
            case "pessimistic_write":
                if (driver instanceof MysqlDriver_1.MysqlDriver ||
                    driver instanceof AuroraDataApiDriver_1.AuroraDataApiDriver ||
                    driver instanceof PostgresDriver_1.PostgresDriver ||
                    driver instanceof OracleDriver_1.OracleDriver) {
                    return " FOR UPDATE";
                }
                else if (driver instanceof SqlServerDriver_1.SqlServerDriver) {
                    return "";
    ...
    SelectQueryBuilder.prototype.createHavingExpression = function () {
        ...
        if (!this.expressionMap.havings || !this.expressionMap.havings.length)
            return "";
        var conditions = this.expressionMap.havings
            .map(function (having, index) {
            switch (having.type) {
                case "and":
                    return ((index > 0 ? "AND " : "") +
                        _this.replacePropertyNames(having.condition));
                case "or":
                    return ((index > 0 ? "OR " : "") +
                        _this.replacePropertyNames(having.condition));
                default:
    ...
    SelectQueryBuilder.prototype.buildEscapedEntityColumnSelects = function (aliasName, metadata) {
        ...
        if (columns.length === 0)
            ...
            return [];
        var nonSelectedPrimaryColumns = this.expressionMap.queryEntity
            ? metadata.primaryColumns.filter(function (primaryColumn) { return columns.indexOf(primaryColumn) === -1; })
            : [];
        ...
        return allColumns.map(function (column) {
            ...
            if (_this.connection.driver.spatialTypes.indexOf(column.type) !== -1) {
                if (_this.connection.driver instanceof MysqlDriver_1.MysqlDriver ||
                    ...
                    var useLegacy = _this.connection.driver.options
            ...
            return {
                ...
                aliasName: selection && selection.aliasName
                    ? selection.aliasName
                    : DriverUtils_1.DriverUtils.buildColumnAlias(_this.connection.driver, aliasName, column.databaseName),
        ...
        if (this.expressionMap.mainAlias.hasMetadata) {
            ...
            if (this.expressionMap.eagerRelations === true) {
                var manuallyJoinedRelations_1 = this.expressionMap.joinAttributes
                    ...
                    .map(function (join) {
                    return join.parentAlias + "." + join.relationPropertyPath;
                });
                                ...
                                countSql =
                                    "COUNT(DISTINCT(" +
                                        metadata.primaryColumns
                                            .map(function (primaryColumn, index) {
    ...
    SelectQueryBuilder.prototype.executeEntitiesAndRawResults = function (queryRunner) {
        return tslib_1.__awaiter(this, void 0, void 0, function () {
            var cloneQb1, cloneQb2, metadata, relationIdLoader, relationCountLoader, relationIdMetadataTransformer, relationCountMetadataTransformer, rawResults, entities, _a, selects, orderBys_1, metadata_1, mainAliasName_1, querySelects, clonnedQb, condition, parameters_1, ids, areAllNumbers, paramAndValues, rawRelationIdResults, rawRelationCountResults, transformer, broadcastResult;
            var _this = this;
            return tslib_1.__generator(this, function (_b) {
                switch (_b.label) {
                    case 0:
                        ...
                        if (!((this.expressionMap.skip || this.expressionMap.take) &&
                            this.expressionMap.joinAttributes.length > 0)) return [3 /*break*/, 4];
                        ...
                        querySelects = metadata_1.primaryColumns.map(function (primaryColumn) {
                            ...
                            var columnAlias = _this.escape(DriverUtils_1.DriverUtils.buildColumnAlias(_this.connection.driver, mainAliasName_1, primaryColumn.databaseName, { extraNeededLength: 4 }));
                            ...
                            return distinctAlias + "." + columnAlias + " as \"ids_" + DriverUtils_1.DriverUtils.buildColumnAlias(_this.connection.driver, mainAliasName_1, primaryColumn.databaseName, { extraNeededLength: 4 }) + "\"";
                        });
                        clonnedQb = cloneQb1
                            // .setFindOptions({
                    ...
                    case 1:
                        ...
                        if (metadata_1.hasMultiplePrimaryKeys) {
                            condition = rawResults
                                ...
                                return metadata_1.primaryColumns
                                    .map(function (primaryColumn) {
                                    parameters_1["ids_" + index + "_" + primaryColumn.databaseName] =
                                        result["ids_" + mainAliasName_1 + "_" + primaryColumn.databaseName];
                            ...
                            ids = rawResults.map(function (result) {
                                return result["ids_" +
                                    DriverUtils_1.DriverUtils.buildColumnAlias(_this.connection.driver, mainAliasName_1, metadata_1.primaryColumns[0].databaseName, { extraNeededLength: 4 })];
                            });
                            areAllNumbers = ids.every(function (id) { return typeof id === "number"; });
                            paramAndValues = DriverUtils_1.DriverUtils.buildParamAndValuesForInClause(this.connection.driver, mainAliasName_1 + "." + metadata_1.primaryColumns[0].propertyPath, ids);
                            if (areAllNumbers) {
                                // fixes #190. if all numbers then its safe to perform query without parameter
                                condition = paramAndValues.param + " IN (" + paramAndValues.values.join(", ") + ")";
                            }
                            else {
                                parameters_1["ids"] = paramAndValues.values;
    ...
    SelectQueryBuilder.prototype.createOrderByCombinedWithSelectExpression = function (parentAlias) {
        ...
        var selectString = Object.keys(orderBys)
            ...
            if (orderCriteria.indexOf(".") !== -1) {
                ...
                return (_this.escape(parentAlias) +
                    "." +
                    _this.escape(DriverUtils_1.DriverUtils.buildColumnAlias(_this.connection.driver, aliasName, column.databaseName)));
            }
            else {
                if (_this.expressionMap.selects.find(function (select) {
                    return select.selection === orderCriteria ||
        ...
        Object.keys(orderBys).forEach(function (orderCriteria) {
            if (orderCriteria.indexOf(".") !== -1) {
                ...
                var alias = _this.expressionMap.findAliasByName(aliasName);
                var column = alias.metadata.findColumnWithPropertyPath(propertyPath);
                orderByObject[_this.escape(parentAlias) +
                    "." +
                    _this.escape(DriverUtils_1.DriverUtils.buildColumnAlias(_this.connection.driver, aliasName, column.databaseName))] = orderBys[orderCriteria];
            }
            else {
                if (_this.expressionMap.selects.find(function (select) {
                    return select.selection === orderCriteria ||
            ...
            return tslib_1.__generator(this, function (_b) {
                switch (_b.label) {
                    case 0:
                        ...
                        cacheOptions = typeof this.connection.options.cache === "object"
                            ? this.connection.options.cache
                            : {};
                        savedQueryResultCacheOptions = undefined;
                        if (!(this.connection.queryResultCache &&
                            (this.expressionMap.cache || cacheOptions.alwaysEnabled))) return [3 /*break*/, 2];
                    ...
                    case 3:
                        results = _b.sent();
                        if (!(this.connection.queryResultCache &&
                            (this.expressionMap.cache || cacheOptions.alwaysEnabled))) return [3 /*break*/, 5];
    ...
    SelectQueryBuilder.prototype.buildSelect = function (select, metadata, alias, embedPrefix) {
        ...
        if (Array.isArray(select)) {
            select.forEach(function (select) {
                _this.selects.push(_this.expressionMap.mainAlias.name +
                    "." +
                    select);
            });
        }
    ...
    SelectQueryBuilder.prototype.buildOrder = function (order, metadata, alias, embedPrefix) {
        var _loop_1 = function (key) {
            ...
            if (!embed && !column && !relation)
                throw new FindCriteriaNotFoundError_1.FindCriteriaNotFoundError(propertyPath, metadata);
            if (column) {
                var direction = order[key] instanceof Object
                    ? order[key].direction
                    : order[key];
                direction =
                    direction === "DESC" ||
                        ...
                        ? "DESC"
                        : "ASC";
                var nulls = order[key] instanceof Object
                    ? order[key].nulls
                    : undefined;
                nulls =
                    nulls === "first"
                ...
                // const joinAlias = alias + "_" + relation.propertyName;
                var joinAlias_1 = DriverUtils_1.DriverUtils.buildColumnAlias(this_1.connection.driver, alias, relation.propertyName);
    ...
    SelectQueryBuilder.prototype.buildWhere = function (where, metadata, alias, embedPrefix) {
        var _this = this;
        var condition = "";
        var parameterIndex = Object.keys(this.expressionMap.nativeParameters)
            .length;
        if (Array.isArray(where)) {
            condition =
                "(" +
                    where
                        .map(function (whereItem) {
                        return _this.buildWhere(whereItem, metadata, alias, embedPrefix);
                    })
                        .filter(function (condition) { return !!condition; })
            ...
            var _loop_2 = function (key) {
                ...
                if (column) {
                    ...
                    var parameterValue = column.transformer
                        ? ApplyValueTransformers_1.ApplyValueTransformers.transformTo(column.transformer, where[key])
                        : where[key];
                    if (parameterValue === null) {
                        ...
                        if (parameterValue.useParameter) {
                            var realParameterValues = parameterValue.multipleParameters
                                ? parameterValue.value
                                : [parameterValue.value];
                            // Hack Julien (Fix the parameter too long issue we have on oracle (limit is 30)
                            // const parameterName = alias + "_" + propertyPath.split(".").join("_") + "_" + parameterIndex;
                            var extraNeededLength = ("" + realParameterValues.length)
                                .length;
                            var parameterName_1 = DriverUtils_1.DriverUtils.buildColumnAlias(this_2.connection.driver, alias, propertyPath.split(".").join("_") +
                                "_" +
                                parameterIndex, { extraNeededLength: extraNeededLength });
                            realParameterValues.forEach(function (realParameterValue, realParameterValueIndex) {
                                ...
                                if (typeof realParameterValue === "number") {
                                    ...
                                    _this.expressionMap.nativeParameters[parameterName_1 +
                                        realParameterValueIndex] = realParameterValue;
                                    parameterIndex++;
                                    parameters_2.push(_this.connection.driver.createParameter(parameterName_1 +
                        ...
                        // Hack Julien (Fix the parameter too long issue we have on oracle (limit is 30)
                        // const parameterName = alias + "_" + propertyPath.split(".").join("_") + "_" + parameterIndex;
                        var parameterName2 = DriverUtils_1.DriverUtils.buildColumnAlias(this_2.connection.driver, alias, propertyPath.split(".").join("_") +
                            "_" +
                            parameterIndex);
                        this_2.expressionMap.nativeParameters[parameterName2] = parameterValue;
                        parameterIndex++;
                        var parameter = this_2.connection.driver.createParameter(parameterName2, parameterIndex - 1);
                    ...
                    if (where[key] instanceof FindOperator_1.FindOperator) {
                        if (where[key].type === "moreThan" ||
                            ...
                            if (relation.isManyToManyOwner) {
                                ...
                                qb.select("COUNT(*)")
                                    ...
                                    .where(relation
                                    .inverseRelation.inverseJoinColumns.map(function (column) {
                                ...
                                qb.select("COUNT(*)")
                                    ...
                                    .where(relation
                                    .inverseRelation.joinColumns.map(function (column) {
                                    return relation
                        ...
                        // Hack Julien (Fix the parameter too long issue we have on oracle (limit is 30)
                        // const joinAlias = alias + "_" + relation.propertyName;
                        var joinAlias_2 = DriverUtils_1.DriverUtils.buildColumnAlias(this_2.connection.driver, alias, relation.propertyName);
