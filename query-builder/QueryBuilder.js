...
var QueryBuilder = /** @class */ (function () {
    ...
    QueryBuilder.prototype.select = function (selection, selectionAliasName) {
        ...
        if (Array.isArray(selection)) {
            this.expressionMap.selects = selection.map(function (selection) { return ({
                selection: selection
            }); });
        }
        else if (selection) {
        ...
        var SelectQueryBuilderCls = require("./SelectQueryBuilder")
            .SelectQueryBuilder;
    ...
    QueryBuilder.prototype.insert = function () {
        ...
        // loading it dynamically because of circular issue
        var InsertQueryBuilderCls = require("./InsertQueryBuilder")
            .InsertQueryBuilder;
    ...
    QueryBuilder.prototype.update = function (entityOrTableNameUpdateSet, maybeUpdateSet) {
        var updateSet = maybeUpdateSet
            ? maybeUpdateSet
            : entityOrTableNameUpdateSet;
        entityOrTableNameUpdateSet =
            entityOrTableNameUpdateSet instanceof __1.EntitySchema
        ...
        // loading it dynamically because of circular issue
        var UpdateQueryBuilderCls = require("./UpdateQueryBuilder")
            .UpdateQueryBuilder;
    ...
    QueryBuilder.prototype.delete = function () {
        ...
        // loading it dynamically because of circular issue
        var DeleteQueryBuilderCls = require("./DeleteQueryBuilder")
            .DeleteQueryBuilder;
    ...
    QueryBuilder.prototype.relation = function (entityTargetOrPropertyPath, maybePropertyPath) {
        ...
        var propertyPath = arguments.length === 2
            ? maybePropertyPath
        ...
        // loading it dynamically because of circular issue
        var RelationQueryBuilderCls = require("./RelationQueryBuilder")
    ...
    QueryBuilder.prototype.createFromAlias = function (entityTarget, aliasName) {
        ...
        if (this.connection.hasMetadata(entityTarget)) {
            ...
            var isSubQuery = entityTarget instanceof Function ||
    ...
    QueryBuilder.prototype.replacePropertyNames = function (statement) {
        ...
        this.expressionMap.aliases.forEach(function (alias) {
            if (!alias.hasMetadata)
                return;
            var replaceAliasNamePrefix = _this.expressionMap
                .aliasNamePrefixingEnabled
                ? alias.name + "\\."
                : "";
            var replacementAliasNamePrefix = _this.expressionMap
                .aliasNamePrefixingEnabled
            ...
            alias.metadata.columns.forEach(function (column) {
                var expression = "([ =(]|^.{0})" +
                    ...
                    column.propertyPath +
                    "([ =),]|.{0}$)";
                statement = statement.replace(new RegExp(expression, "gm"), "$1" +
                    replacementAliasNamePrefix +
                    _this.escape(column.databaseName) +
                    "$2");
                var expression2 = "([ =(]|^.{0})" +
                    replaceAliasNamePrefix +
                    column.propertyName +
                    "([ =),]|.{0}$)";
                statement = statement.replace(new RegExp(expression2, "gm"), "$1" +
                    replacementAliasNamePrefix +
                    _this.escape(column.databaseName) +
                    "$2");
            });
            alias.metadata.relations.forEach(function (relation) {
                tslib_1.__spread(relation.joinColumns, relation.inverseJoinColumns).forEach(function (joinColumn) {
                    ...
                    statement = statement.replace(new RegExp(expression, "gm"), "$1" +
                        replacementAliasNamePrefix +
                        _this.escape(joinColumn.databaseName) +
                        "$2"); // todo: fix relation.joinColumns[0], what if multiple columns
                });
                if (relation.joinColumns.length > 0) {
                    ...
                    statement = statement.replace(new RegExp(expression, "gm"), "$1" +
                        replacementAliasNamePrefix +
                        _this.escape(relation.joinColumns[0].databaseName) +
                        "$2"); // todo: fix relation.joinColumns[0], what if multiple columns
                }
            });
        });
    ...
    QueryBuilder.prototype.createWhereExpression = function () {
        ...
        if (!conditions.length)
            ...
            return this.expressionMap.extraAppendedAndWhereCondition
                ? " WHERE " +
                    this.replacePropertyNames(this.expressionMap.extraAppendedAndWhereCondition)
                : "";
        ...
        if (columns.length) {
            var columnsExpression = columns
                ...
                if (driver instanceof SqlServerDriver_1.SqlServerDriver) {
                    if (_this.expressionMap.queryType === "insert" ||
                        _this.expressionMap.queryType === "update") {
                        return "INSERTED." + name;
                    }
                    else {
                        return (_this.escape(_this.getMainTableName()) +
                            "." +
                            name);
                    }
                }
                ...
                columnsExpression +=
                    " INTO " +
                        columns
                            ...
                            var parameterName = DriverUtils_1.DriverUtils.buildColumnAlias(_this.connection.driver, "output", column.databaseName);
                            ...
                            return _this.connection.driver.createParameter(parameterName, Object.keys(_this.expressionMap.nativeParameters)
                                .length);
                        })
                            .join(", ");
            }
    ...
    QueryBuilder.prototype.createWhereExpressionString = function () {
        ...
        return this.expressionMap.wheres
            .map(function (where, index) {
            switch (where.type) {
                case "and":
                    return ((index > 0 ? "AND " : "") +
                        _this.replacePropertyNames(where.condition));
                case "or":
                    return ((index > 0 ? "OR " : "") +
                        _this.replacePropertyNames(where.condition));
                default:
    ...
    QueryBuilder.prototype.createWhereIdsExpression = function (ids) {
        ...
        if (!metadata.hasMultiplePrimaryKeys &&
            ...
            if (!primaryColumn_1.transformer) {
                return this.computeWhereParameter((_a = {},
                    _a[primaryColumn_1.propertyName] = In_1.In(normalized.map(function (id) {
        ...
        var alias = this.expressionMap.aliasNamePrefixingEnabled
        ...
        if (metadata.primaryColumns.length > 1) {
            var whereStrings = normalized.map(function (id, index) {
                ...
                metadata.primaryColumns.forEach(function (primaryColumn, secondIndex) {
                    ...
                    whereSubStrings.push(alias +
                        _this.escape(primaryColumn.databaseName) +
                        " = " +
                        _this.connection.driver.createParameter(parameterName, parameterIndex));
            ...
            return whereStrings.length > 1
                ? "(" +
                    whereStrings
                        .map(function (whereString) { return "(" + whereString + ")"; })
                        .join(" OR ") +
                    ")"
                : whereStrings[0];
        }
            ...
            var normalizedValues = normalized.map(function (ent) { return ent[Object.keys(ent)[0]]; });
            var areAllNumbers = normalizedValues.every(function (id) { return typeof id === "number"; });
            var paramAndValues = void 0;
            if (areAllNumbers) {
                paramAndValues = DriverUtils_1.DriverUtils.buildParamAndValuesForInClause(this.connection.driver, "" + (alias + this.escape(primaryColumn_2.databaseName)), normalizedValues);
                return paramAndValues.param + " IN (" + paramAndValues.values.join(", ") + ")";
            }
            else {
                paramAndValues = DriverUtils_1.DriverUtils.buildParamAndValuesForInClause(this.connection.driver, "" + (alias + this.escape(primaryColumn_2.databaseName)), normalized.map(function (val) {
                    return primaryColumn_2.getEntityValue(val, true);
                }));
    ...
    QueryBuilder.prototype.computeWhereParameter = function (where) {
        ...
        if (where instanceof Brackets_1.Brackets) {
            ...
            var wheres = Array.isArray(where)
                ? where
                : [where];
            ...
            if (this.expressionMap.mainAlias.hasMetadata) {
                andConditions = wheres.map(function (where, whereIndex) {
                    ...
                    return propertyPaths
                        .map(function (propertyPath, propertyIndex) {
                        ...
                        return columns
                            ...
                            var aliasPath = _this.expressionMap
                                .aliasNamePrefixingEnabled
                                ? _this.alias + "." + propertyPath
                                : column.propertyPath;
                            var parameterValue = column.getEntityValue(where, true);
                            if (parameterValue === undefined)
                                return;
                            var parameterName = "where_" +
                                whereIndex +
                                "_" +
                                propertyIndex +
                                "_" +
                                columnIndex;
                            var parameterBaseCount = Object.keys(_this.expressionMap.nativeParameters).filter(function (x) { return x.startsWith(parameterName); })
                                .length;
                                ...
                                if (parameterValue.useParameter) {
                                    ...
                                    realParameterValues.forEach(function (realParameterValue, realParameterValueIndex) {
                                        ...
                                        if (typeof realParameterValue ===
                                            ...
                                            parameterIndex_1++;
                                            parameters_1.push(_this.connection.driver.createParameter(parameterName +
                                                (parameterBaseCount +
                                                    realParameterValueIndex), parameterIndex_1 -
                                                1));
                                        }
                                    });
                                }
                ...
                andConditions = wheres.map(function (where, whereIndex) {
                    return Object.keys(where)
                        .map(function (key, parameterIndex) {
            ...
            if (andConditions.length > 1)
                return andConditions
                    .map(function (where) { return "(" + where + ")"; })
                    .join(" OR ");
