...
var RelationIdLoader = /** @class */ (function () {
                        ...
                        return [2 /*return*/, entities.map(function (entity) {
                                ...
                                var entityRelationIds = relationIds.filter(function (relationId) {
                                    return inverseColumns.every(function (column) {
                                        ...
                                        var columnName = DriverUtils.buildColumnAlias(_this.connection.driver, column.entityMetadata.name +
                                            "_" +
    ...
    RelationIdLoader.prototype.loadForManyToMany = function (relation, entities, relatedEntities) {
        ...
        columns.forEach(function (column) {
            ...
            // Hack Julien (Fix the parameter too long issue we have on oracle (limit is 30)
            columnName = DriverUtils.buildColumnAlias(_this.connection.driver, columnName);
        ...
        if (columns.length === 1) {
            ...
            var paramAndValues = DriverUtils.buildParamAndValuesForInClause(this.connection.driver, mainAlias + "." + columns[0].propertyPath, values);
            if (areAllNumbers) {
        ...
        if (relatedEntities) {
            if (inverseColumns.length === 1) {
                ...
                var areAllNumbers = values.every(function (value) { return typeof value === "number"; });
                var paramAndValues = DriverUtils.buildParamAndValuesForInClause(this.connection.driver, mainAlias + "." + inverseColumns[0].propertyPath, values);
                if (areAllNumbers) {
    ...
    RelationIdLoader.prototype.loadForManyToOneAndOneToOneOwner = function (relation, entities, relatedEntities) {
        ...
        if (relation.entityMetadata.primaryColumns.length === 1) {
            ...
            var areAllNumbers = values.every(function (value) { return typeof value === "number"; });
            var paramAndValues = DriverUtils.buildParamAndValuesForInClause(this.connection.driver, mainAlias + "." + relation.entityMetadata.primaryColumns[0].propertyPath, values);
            if (areAllNumbers) {
    ...
    RelationIdLoader.prototype.loadForOneToManyAndOneToOneNotOwner = function (relation, entities, relatedEntities) {
        ...
        if (relation.joinColumns.length === 1) {
            ...
            var areAllNumbers = values.every(function (value) { return typeof value === "number"; });
            var paramAndValues = DriverUtils.buildParamAndValuesForInClause(this.connection.driver, mainAlias + "." + relation.joinColumns[0].propertyPath, values);
            if (areAllNumbers) {
                condition = paramAndValues.param + " IN (" + paramAndValues.values.join(", ") + ")";
            }
            else {
