...
var RelationLoader = /** @class */ (function () {
    ...
    RelationLoader.prototype.loadManyToOneOrOneToOneOwner = function (relation, entityOrEntities, queryRunner, queryBuilder) {
        ...
        if (columns.length === 1) {
            ...
            var paramAndValues = DriverUtils.buildParamAndValuesForInClause(this.connection.driver, joinAliasName + "." + columns[0].propertyPath, values);
            if (areAllNumbers) {
                ...
                var parameterName = DriverUtils.buildColumnAlias(this.connection.driver, joinAliasName, columns[0].propertyName);
            ...
            var condition = entities
                ...
                return columns
                    ...
                    var paramName = DriverUtils.buildColumnAlias(_this.connection.driver, joinAliasName, "entity_" + entityIndex + "_" + columnIndex);
    ...
    RelationLoader.prototype.loadOneToManyOrOneToOneNotOwner = function (relation, entityOrEntities, queryRunner, queryBuilder) {
        ...
        if (columns.length === 1) {
            ...
            var areAllNumbers = values.every(function (value) { return typeof value === "number"; });
            var paramAndValues = DriverUtils.buildParamAndValuesForInClause(this.connection.driver, aliasName + "." + columns[0].propertyPath, values);
            if (areAllNumbers) {
                qb.where(paramAndValues.param + " IN (" + paramAndValues.values.join(", ") + ")");
            }
            else {
                ...
                var parameterName = DriverUtils.buildColumnAlias(this.connection.driver, aliasName, columns[0].propertyName);
                qb.where(paramAndValues.param + " IN (:..." + parameterName + ")");
                qb.setParameter(parameterName, paramAndValues.values);
            }
        }
            ...
            var condition = entities
                ...
                return columns
                    .map(function (column, columnIndex) {
                    // Hack Julien (Fix the parameter too long issue we have on oracle (limit is 30)
                    var paramName = DriverUtils.buildColumnAlias(_this.connection.driver, aliasName, "entity_" + entityIndex + "_" + columnIndex);
    ...
    RelationLoader.prototype.loadManyToManyOwner = function (relation, entityOrEntities, queryRunner, queryBuilder) {
        ...
        relation.joinColumns.forEach(function (joinColumn) {
            ...
            var areAllNumbers = values.every(function (value) { return typeof value === "number"; });
            var paramAndValues = DriverUtils.buildParamAndValuesForInClause(_this.connection.driver, joinAlias + "." + joinColumn.propertyName, values);
            if (areAllNumbers) {
    ...
    RelationLoader.prototype.loadManyToManyNotOwner = function (relation, entityOrEntities, queryRunner, queryBuilder) {
        ...
        relation.inverseRelation.inverseJoinColumns.forEach(function (column) {
            ...
            var areAllNumbers = values.every(function (value) { return typeof value === "number"; });
            var paramAndValues = DriverUtils.buildParamAndValuesForInClause(_this.connection.driver, joinAlias + "." + column.propertyName, values);
            if (areAllNumbers) {
                joinColumnConditions.push(paramAndValues.param + " IN (" + paramAndValues.values.join(", ") + ")");
            }
            else {
