 ...
 */
export function createLiteralTreeRepository(_a) {
    ...
    // -------------------------------------------------------------------------
    return tslib_1.__assign({}, createLiteralRepository({ manager: manager, target: target, queryRunner: queryRunner }), { findTrees: function () {
        ...
        createDescendantsQueryBuilder: function (alias, closureTableAlias, entity) {
            ...
            if (this.getMetadata().treeType === "closure-table") {
                ...
                var joinCondition = this.getMetadata().treeParentRelation.joinColumns.map(function (joinColumn) {
                    ...
                    var parameterName = DriverUtils.buildColumnAlias(_this.manager.connection.driver, joinColumn.referencedColumn.propertyPath.replace(".", "_"));
