...
var EntityManager = /** @class */ (function () {
    ...
    EntityManager.prototype.getRepository = function (target) {
        ...
        var repository = this.repositories.find(function (repository) { return repository.target === target; });
        if (repository)
            return repository;
        // if repository was not found then create it, store its instance and return it
        if (this.connection.driver instanceof MongoDriver) {
            var newRepository = createLiteralMongoRepository({
                manager: this,
                target: target,
            ...
            var newRepository = createLiteralRepository({
                manager: this,
                target: target,
                queryRunner: this.queryRunner,
            });
    ...
    EntityManager.prototype.getTreeRepository = function (target) {
        ...
        // find already created repository instance and return it if found
        var repository = this.treeRepositories.find(function (repository) { return repository.target === target; });
        if (repository)
            return repository;
        // check if repository is real tree repository
        var newRepository = createLiteralTreeRepository({
            manager: this,
            target: target,
            queryRunner: this.queryRunner,
        });
